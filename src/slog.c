...
static logger_cfg g_logger_cfg = {NULL, NULL, {0}, S_INFO, FALSE};
...
static void _get_curr_date(int datestr_size, char datestr[]) {
  ...
  snprintf(datestr, datestr_size - 1, DATE_STR_FMT, curr_time->tm_year + 1900,
           curr_time->tm_mon + 1, curr_time->tm_mday);
}
...
static void _get_curr_time(int timestr_size, char timestr[]) {
  ...
  time(&tt);
  curr_time = localtime(&tt);
  snprintf(timestr, timestr_size, TIME_STR_FMT, curr_time->tm_year + 1900,
...
static void _write_stacktrace() {
  ...
  symbol = (SYMBOL_INFO *)calloc(
      sizeof(SYMBOL_INFO) + sizeof(char) * MAX_ST_INFO, 1);
...
int init_logger(const char *log_dir, slog_level level) {
  ...
  snprintf(log_filepath, sizeof(log_filepath) - 1, "%s/%s.log", log_dir,
           datestr);
...
void write_log(slog_level level, int print_stacktrace, const char *func_name,
  ...
  snprintf(log_line, sizeof(log_line) - 1, "%s[%s][%s][%s:%d]%s", level_str,
           timestr, file_name, func_name, line, log_content);
